/*
 * Film 4b — connect-timeline-v2  (a kept-separate upgrade of connect-timeline)
 *
 * Upgrades over the original (flow/sequencing modelled on capture-photo-v2):
 *  - A time indicator (clock + start time) marks the left origin of the axis.
 *  - Callouts land strictly one-at-a-time, evenly staggered — never overlapping.
 *  - No loose dots: every node on the line is wired to a callout; the old
 *    decorative "history" dots are gone, and correlation rings sit only on
 *    selected nodes.
 *  - The selection box fully contains the items it groups (icons above + tags
 *    below, through 1450 RPM) and carries a faint tint to read as "selected".
 *  - OPERATOR CONTEXT / MACHINE DATA are fixed header labels (the camera zoom
 *    never crops them) each fronted by a filled glyph — a hard-hat operator
 *    silhouette and a small machine.
 *
 * The original `connect-timeline` composition is left untouched.
 */
...
// correlation rings — only on selected line-nodes inside the box
...
// selection box — fully contains the photo/voice icons (top) and the
// 2.4 bar / 1450 RPM tags (bottom), through the right edge of 1450 RPM
const BOX = { x: 360, y: 58, w: 422, h: 260 };

// status checklist, stacked and centred under the box (box centre x ≈ 571)
const STATUS: Array<{ label: string; color: string; start: number; dim?: boolean }> = [
  { label: "CORRELATION DETECTED", color: HUMAN, start: 285 },
  { label: "GENERATING INSIGHT", color: INK, start: 320, dim: true },
  { label: "VERIFIED RESULT", color: VERIFIED, start: 352 },
];
...
export const ConnectTimelineV2: React.FC = () => {
  ...
  const cluster = easeOut(frame, 255, 26);
  ...
  return (
    <Film viewW={960} viewH={420}>
      <g transform={`scale(${scale})`} style={{ transformOrigin: "480px 210px" }}>
        ...
        {/* selection box (drawn before the points so the tint sits behind them) */}
        <g
          opacity={cluster}
        ...
        {/* correlation rings breathe, then hold steady — only on selected nodes */}
        {RINGS.map(([cx, cy], i) => {
          const start = 285 + i * 4;
          const t = frame - start;
          if (t < 0) return null;
      ...
      {/* status checklist — stacked and centred under the selection box */}
      {STATUS.map((r, i) => {
